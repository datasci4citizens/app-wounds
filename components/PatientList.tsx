    ...
    <div className="border border-border rounded-xl bg-muted/5 overflow-hidden">
      <div className="p-4">
        ...
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
          {age !== null && <Chip icon={<Calendar className="w-2.5 h-2.5" />} text={`${age}a`} />}
          {patient.gender && <Chip text={formatGender(patient.gender)} />}
          {(patient.city || patient.state) && (
            <Chip icon={<MapPin className="w-2.5 h-2.5" />} text={`${patient.city || ''}${patient.state ? `-${patient.state}` : ''}`} />
          )}
          {patient.height != null && <Chip icon={<Ruler className="w-2.5 h-2.5" />} text={`${patient.height}m`} />}
          {patient.weight != null && <Chip icon={<Weight className="w-2.5 h-2.5" />} text={`${patient.weight}kg`} />}
          {smoking && <Chip icon={<Wind className="w-2.5 h-2.5" />} text={smoking} />}
          {alcohol && <Chip icon={<Wine className="w-2.5 h-2.5" />} text={alcohol} />}
