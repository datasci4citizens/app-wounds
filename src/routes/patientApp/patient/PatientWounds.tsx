// ATENÇÃO: Este arquivo está usando dados mockados para desenvolvimento
// Para voltar ao comportamento normal, substitua as funções customGetRequest e customDeleteRequest
// pelas versões originais que fazem chamadas reais para a API
...
const mockPatient: Patient = {
    ...
    hospital_registration: "HSP001234",
    phone_number: "(11) 99999-9999",
    height: 165,
    weight: 70,
    smoke_frequency: "Nunca fumou",
    drink_frequency: "Socialmente",
    accept_tcle: true,
    specialist_id: 1,
    comorbidities: [1, 3], // Diabetes e Hipertensão
...
const customGetRequest = async (url: string) => {
    ...
    if (url.includes('/wounds?patient_id=')) {
        return mockWounds;
    }
    
    if (url.includes('/comorbidities')) {
        return Object.entries(mockComorbidities).map(([id, name]) => ({
            comorbidity_id: parseInt(id),
            name
        }));
    }
    
    // Fallback para outras chamadas (manter comportamento original se necessário)
...
// Define a custom DELETE request function (mockado)
const customDeleteRequest = async (url: string) => {
    // Simular delay de rede
    await new Promise(resolve => setTimeout(resolve, 1000));
    
    console.log(`Mock: Deletando ferida via URL: ${url}`);
    
    // Simular sucesso na exclusão
    return true;
};
